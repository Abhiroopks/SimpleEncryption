...
//TODO : fix the ptrs (make sure data is circularly accessed)
...
int main (int argc, char *argv[]) {
	...
	//Segment shuffling
	//split the data into equal size segments (2^8 segments)
	int numSegments = (1 << 8);
	...
	for(int i = 0; i < numSegments; i++){
		swapSegInd = (uint8_t)keybuff[i % keylen];
		swapSegs(inbuff,(i*segmentSize) % inlen, swapSegInd % inlen,segmentSize,inlen);
	}
